...
description: "添加了一个代码自动格式化小工具 prettier-plugin-sort-imports react 和 vue 模板都已配置 格式化前： 格式化后 插件配置位置：.prettierrc.cjs"
category: "TOOLING"
...
readingTime: 1
date: "2023-04-20"
title: "添加引入排序自动格式化插件 （已移除，现在统一使用eslint管理格式化）"
visual: "conference"
---
# ~~~添加引入排序自动格式化插件~~~（已移除，现在统一使用eslint管理格式化）
