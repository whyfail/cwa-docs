...
description: "其实关于项目中添加 pxtorem，这个配置褒贬不一，对于一些特殊项目，比如设计时候以大屏为主的比较适合这样的配置，其他时候其实不见得有这个配置就是好事，毕竟用 4k 屏是为了看更多的内容，而不是更大的字，所以有必要说明一下如何去..."
category: "UI / STYLING"
...
## 一、环境变量控制
- 在.env文件中修改`VITE_USE_REM`变量,如下配置：
```js
VITE_USE_REM=false
```
- 当`VITE_USE_REM`为`false`时，pxtorem 配置不会生效（默认为true）
